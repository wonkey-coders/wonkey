...
Class Tile Extends GameObject
	...
	'The location of the tile in the map
	Field Location:MapNode
...
Class TileMap
	...
	'Get the nearest node to a point on a tile
	Method GetNearestNodeToTile:MapNode(point:Vec2f, tile:MapNode)
		Local distance:Float = 9999999
		Local node:MapNode
		'We need to check the adjacent nodes to the tile - top left, top right, bottom left and bottom right
		'Use these for..next loops to do that
		For Local x:Int = 0 To -1 Step -1
			For Local y:Int = 0 To -1 Step -1
				'Make sure the coordinates are within the bounds of the map to avoid array out of bounds errors
				If tile.XY.x + x > 0 And tile.XY.x + x < Cols-1 And tile.XY.y + y > 0 And tile.XY.y + y < Rows-1
					'Does a node exist at these coordinates?
					If Layers[tile.Layer].Nodes[tile.XY.x + x, tile.XY.y + y]
					    'If so get the world coordinates of the node and calculate the distance to the point we're checking
						Local vec:=New Vec2f(NodeXToWorld(tile.XY.x + x), NodeYToWorld(tile.XY.y + y))
						'Use Abs because we don't want negative numbers
						Local length:=Abs<Double>((vec - point).Length)
						'Keep reducing the distance until we find the shortest one
						If length < distance
							node = New MapNode(tile.XY.x + x, tile.XY.y + y, tile.Layer)
							distance = length
						End If
					End If
				End If
			Next
		Next
		'If distnace is still 9999999 then no node was found so return -1. In theory a tile cannot exist on the map with a node
		'so if this happens something has gone wrong elsewhere!
		If distance = 9999999 Return New MapNode(-1, -1, -1)
		'Return the nearest node that was found
		Return node
	End
...
Class TileLayer
	...
	Field Index:Int
	'The terrain stats of this layer
	Field Properties:TerrainProperties = New TerrainProperties(120)
	'The rock object image to use for the debris after a map node is destroyed
	Field RockImage:Image
	...
	Method New(map:TileMap)
		...
		Nodes = New TerrainProperties[Map.Cols, Map.Rows]
	End

	Method SetTerrainProperties(health:Float)
		Properties = New TerrainProperties(health)
	End
	...
	Method Paint(x:Int, y:Int)
		...
		If x>0 And x<Map.Cols-1 And y>0 And y<Map.Rows-1
			If Not Nodes[x, y]
				...
				Nodes[x, y] = New TerrainProperties(Properties.Health)
			End If
		End
	End
	...
	Method Erase(x:Int, y:Int)
		...
		If x>0 And x<Map.Cols-1 And y>0 And y<Map.Rows-1
			...
			If Nodes[x, y]
				'Erase the node by setting it to null
				Nodes[x, y] = Null
	...
	Method AssignTiles(xy:Vec2i)
		...
		'Store the grid coordinates of the tile
		Tiles[xy.x, xy.y].Location = New MapNode(xy.x, xy.y, Index)
		...
		Tiles[xy.x + 1, xy.y].Location = New MapNode(xy.x + 1, xy.y, Index)
		...
		Tiles[xy.x + 1, xy.y + 1].Location = New MapNode(xy.x + 1, xy.y + 1, Index)
		...
		Tiles[xy.x, xy.y + 1].Location = New MapNode(xy.x, xy.y + 1, Index)
