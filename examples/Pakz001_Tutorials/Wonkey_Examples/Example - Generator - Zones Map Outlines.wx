...
Class themap
	...
	Method New(mw:Int,mh:Int,numzones:Int)
		...
		outlinemap()
	End Method
	' Here we create a outline map
	Method outlinemap()
		Local map2:Int[,] = New Int[mw,mh]
		...
		For Local y:Int=0 Until mh
		For Local x:Int=0 Until mw
			Local cc:Int=map[x,y]
			Local deleteme:Bool=True
			...
			For Local x1:Int=-1 To 1
				...
				If x+x1>=0 And y+y1>=0 And x+x1<mw And y+y1<mh
					If map[x+x1,y+y1] <> cc Then deleteme = False
				End If
				End If
			Next
			Next
			If deleteme = True Then map2[x,y] = 0
		Next
		Next
		For Local y:Int=0 Until mh
		For Local x:Int=0 Until mw
...
Class MyWindow Extends Window
	...
	Method OnRender( canvas:Canvas ) Override
		...
		cnt+=1
		If Keyboard.KeyReleased(Key.Space) Or Mouse.ButtonHit(MouseButton.Left) Or cnt>180 Then 
			mymap = New themap(Width/6,Height/6,Rnd(2,20))
			cnt=0
		End If
